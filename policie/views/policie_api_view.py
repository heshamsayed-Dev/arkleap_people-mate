...
#         queryset = (
#             self.get_queryset()
#             .filter(Q(end_date__gte=date.today()) | Q(end_date__isnull=True))
#             .order_by("-created_at")
#         )
#         policies = self.filter_queryset(queryset)
#         results = self.paginate_queryset(policies)
#         serializer = self.get_serializer(results, many=True)
#         return Response(data=serializer.data, status=status.HTTP_200_OK, count=results.count())
